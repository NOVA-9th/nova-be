...
public class NewsDebugController {
    ...
    @GetMapping("/newsapi")
    public List<Article> fetchFromNewsApi() { return getServiceByName("NewsAPI").fetchArticles(); }

    @Operation(summary = "[뉴스] Naver News Search 호출", description = "네이버 뉴스 검색 결과를 가져옵니다.")
    @GetMapping("/navernewssearch")
    public List<Article> fetchFromNaverNews() { return getServiceByName("NaverNewsSearch").fetchArticles(); }

    @Operation(summary = "[뉴스] DeepSearch 호출", description = "DeepSearch API에서 데이터를 가져옵니다.")
    @GetMapping("/deepsearch")
    public List<Article> fetchFromDeepSearch() { return getServiceByName("DeepSearch").fetchArticles(); }

    @Operation(summary = "[뉴스] NewsData 호출", description = "NewsData.io에서 데이터를 가져옵니다.")
    @GetMapping("/newsdata")
    public List<Article> fetchFromNewsData() { return getServiceByName("NewsData").fetchArticles(); }

    @Operation(summary = "[뉴스] GNews 호출", description = "GNews API에서 데이터를 가져옵니다.")
    @GetMapping("/gnews")
    public List<Article> fetchFromGNews() { return getServiceByName("GNews").fetchArticles(); }

    @Operation(summary = "[뉴스] Naver News API 호출", description = "네이버 뉴스 공식 API를 통해 데이터를 가져옵니다.")
    @GetMapping("/navernewsapi")
    public List<Article> fetchFromNaverNewsApi() { return getServiceByName("NaverNewsAPI").fetchArticles(); }

    @Operation(summary = "[뉴스] Hacker News 호출", description = "Hacker News의 최신 인기 게시글을 가져옵니다.")
    @GetMapping("/hackernews")
    public List<Article> fetchFromHackerNews() { return getServiceByName("HackerNews").fetchArticles(); }
