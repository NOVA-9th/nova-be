...
public class WebClientConfig {
    ...
    // dev.to 추가
    @Bean(name = "devToWebClient")
    public WebClient devToWebClient(
            @Value("${external.devto.base-url}") String baseUrl,
            @Value("${external.devto.user-agent}") String userAgent
    ) {
        return WebClient.builder()
                .baseUrl(baseUrl) // https://dev.to/api
