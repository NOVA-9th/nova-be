...
*   **Method:** **POST**
*   **설명:** 기사 수집부터 AI 요약, DB 저장까지의 전체 프로세스를 실행합니다. 비동기(`@Async`)로 동작하므로 호출 즉시 응답하며, 작업은 백그라운드에서 진행됩니다.
*   **보안:** 관리자 권한이 필요하며, 일반 사용자에게는 공개되지 않습니다.
...
*   `batch_run_metadata`: 배치의 성공/실패 여부와 증분 수집의 기준이 되는 실행 시각을 기록하는 테이블 (실행 중 중복 방지 기능 포함)
...
*   **중복 실행 방지:** 이미 `RUNNING` 상태인 배치가 있다면 새로운 배치는 실행되지 않고 스킵됩니다.
